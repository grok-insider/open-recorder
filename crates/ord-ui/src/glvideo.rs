...
unsafe fn make_tex(gl: &glow::Context) -> Option<glow::Texture> {
    ...
    // Plain bilinear, like every video player: mipmapped trilinear needed a
    // full mip-chain regeneration of both planes on EVERY uploaded frame
    // (`generate_mipmap` at 60fps) — a steady, measurable GPU cost — and the
    // preview is drawn at/near source scale where mips add nothing.
